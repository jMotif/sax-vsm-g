package net.seninp.saxvsm.gi.sequitur;
...
public final class SequiturFactory {
  ...
  private static final double NORMALIZATION_THRESHOLD = 0.001D;
  ...
  public static int[] series2RulesDensity(double[] originalTimeSeries, int saxWindowSize,
    ...
    for (int i = 0; i < originalTimeSeries.length - (saxWindowSize - 1); i++) {
      ...
      // Z normalize it
      subSection = tsp.znorm(subSection, NORMALIZATION_THRESHOLD);

      // perform PAA conversion if needed
      double[] paa = tsp.paa(subSection, saxPaaSize);

      // Convert the PAA to a string.
      char[] currentString = tsp.ts2String(paa, normalA.getCuts(saxAlphabetSize));
  ...
  public static SAXRecords discretizeNoSlidingWindow(double[] timeseries, int saxPAASize,
    ...
    // Z normalize it
    double[] normalizedTS = tsp.znorm(timeseries, normalizationThreshold);

    // perform PAA conversion if needed
    double[] paa = tsp.paa(normalizedTS, saxPAASize);
  ...
  private static Integer mean(int[] m) {
    long sum = 0;
